...
> **بصراحة التوثيق:** ليست كل الطبقات مفعّلة لكل مدخل. الفاحص يضرب فقط الحقول الموجودة فعلًا في السجل (`url` + `gazette_url` + `istitlaa_url` + `sector_feed`) — والحقل الغائب يُتخطى ولا يُخترع له رابط. الوضع الحالي: **14 نظامًا بصفحات LawDetails محددة** (آخرها: المحاكم التجارية، PDPL، المنافسة، التعاملات الإلكترونية، الوكالات التجارية — وُثقت عبر مطابقة العنوان ورقم المرسوم في نتائج البحث الرسمية) + **3 وثائق قطاعية مسماة** (REAC/SAFF/FIFA). البوابات المتبقية بصراحة (3): الإفلاس (صفحة اللائحة التنفيذية لا تُستخدم بديلًا عن صفحة النظام)، الملكية الفكرية (عنقود لا صفحة واحدة)، المحاماة (المرجع SBA). روابط `gazette_url` المباشرة موثقة لمدخلين (المبلغين، تنفيذية PDPL) — البقية تُضاف فور توثيقها من `uqn.gov.sa`. المدخلات بثقة `portal` إشارتها ضعيفة بالاتجاهين: تغيّرها يُسجَّل `PORTAL_CHANGED` للمعلومية فقط ولا يفتح Issue.
...
| [فرع `regulation-watch-state`](https://github.com/SMSMy/saudi-legal-ai/tree/regulation-watch-state) | مكان حفظ الأساس الفعلي — فرع مستقل غير محمي، لأن `main` محمي ضد دفع البوت (GH013). كل تشغيل يسترجع الأساس منه، ويدفع فوق رأسه مباشرة (fast-forward، لا `checkout -B` أبدًا حتى لا يتشعب التاريخ وينكسر الدفع التالي). الدفع مشروط بتغيّر بصمات الـURLs فعلًا (`signals_snapshot`) — دوران العدّادات وحده (`last_seen` / `consecutive_unreachable`) لا يستحق commit أسبوعيًا |
...
| `URL_RETIRED` | الرابط المراقَب لم يعد يخدم المورد: صفحة خطأ/دخول، أو 404/410، أو رابط مستند (`.pdf`/`.doc`/`.docx`) صار يخدم HTML | **انقطاع تتبّع لا إشارة تعديل** — لا Issue، ولا يجمّد الأساس؛ يلزم توثيق رابط بديل بشريًا، والمصدر يبقى فعليًا بلا مراقبة حتى ذلك |
...
4. **SAFF/FIFA** لوائح موسمية/دولية — الإشارة هنا تنبيه بنشر ملفات جديدة قبل كل موسم، لا حكمًا قانونيًا. **تنبيه ثقة:** فهرس وثائق دولي يبني نفسه بجافاسكربت (`no-store`، `HEAD 405`، ETag يتبدّل أسبوعيًا) **ليس «وثيقة مسماة» بثقة `exact`** — لذلك `fifa_rstp` مُسجَّل `portal` عمدًا (تخفيض إشارة، لا تغيير هدف: `url`/`sector_feed` كما هي). يُعاد النظر فقط عند توثيق رابط ملف RSTP **مستقر**؛ ومعرّفات `digitalhub` تتغيّر مع كل طبعة، فتثبيتها `exact` يعيد مشكلة الرابط الميت.
